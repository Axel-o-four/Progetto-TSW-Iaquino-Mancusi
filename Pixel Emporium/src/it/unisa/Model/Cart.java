...
public class Cart {
    ...
    public void updateProductQuantity(int code, String prefissoId, int newQuantity) {
        Iterator<Item> it = items.iterator();
        while (it.hasNext()) {
            Item item = it.next();
            // confronto sia code sia prefissoId, così identifichiamo esattamente il bean
            if (item.getCode() == code && item.getPrefissoId().equals(prefissoId)) {
                if (newQuantity > 0) {
