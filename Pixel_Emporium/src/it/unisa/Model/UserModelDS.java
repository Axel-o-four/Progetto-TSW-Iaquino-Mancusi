...
public class UserModelDS implements UserModel {
    ...
    public synchronized boolean updateUser(UserBean user) throws SQLException {
        Connection connection = null;
        PreparedStatement preparedStatement = null;
        String updateSQL = "UPDATE utente SET nome=?, cognome=?, data_Nascita=?, genere=?, indirizzo=?, citta=?, prov=?, cap=?, password=? WHERE email=?";
        try {
            connection = ds.getConnection();
            connection.setAutoCommit(false);
            ...
            preparedStatement.setDate(3, new java.sql.Date(user.getDataNascita().getTime()));
            preparedStatement.setString(4, user.getGenere());
            preparedStatement.setString(5, user.getIndirizzo());
            preparedStatement.setString(6, user.getCitta());
            preparedStatement.setString(7, user.getProv());
